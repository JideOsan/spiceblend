...
export default function SpicesFeed({ searchString } : { searchString: string}) {
  ...
  const rowCount = Math.ceil(spices.length / columnCount);
    ...
    useEffect(() => {
      spices.forEach(spice => previousSpicesRef.current.add(spice.id.toString()));
    }, [spices]);
      ...
      {({ height, width }) => (
        ...
        >
          {({ columnIndex, rowIndex, style }) => {
                ...
                const isNewItem = !previousSpicesRef.current.has(spice?.id.toString());
                return spice ? (
                  <motion.div
                    key={spice.id}
                    style={{ ...style }}
                    ...
                    transition={{ duration: 1/*, delay: isNewItem ? (index - spices.length) * 0.05 : 0,*/}}
                    className="flex p-4"
                  >
